...
vcs_23 <- check("Violation Check Samples", 2023) # UNDERSTAND WHERE THESE NUMBERS ARE COMING
...
#Creating data frames of lims numbers with discrepensies in counts

ln_diff <- function(x){
  col <- table_generator(x, "2023", "Coliforms Total (Colilert)")
  cl2 <- table_generator(x, "2023", "Field-Chlorine Residual Total") 
  q1 <- setdiff(col$`Samp #` , cl2$`Samp #`)
  q2 <- setdiff(cl2$`Samp #`, col$`Samp #`)
  
  ln_inspect <- if(is_empty(q1) == F & is_empty(q2) == T) {
    q1
  } else (if(is_empty(q1) == T & is_empty(q2)== F) {
    q2
  } else( if(is_empty(q1)== F & is_empty(q2)== F) {
    q1 +q2
  } else(if(is_empty(q1)== T & is_empty(q2)== T){
    NA
  })))
  
  print(read_LIMS(
    lims_number = ln_inspect,
    parameter = c("Coliforms Total (Colilert)", "Field-Chlorine Residual Total"),
    select_additiona = "METHOD_USED"
  ))
  
}


feb_diff <- ln_diff("February") #fcl2 count is missing a value
mar_diff <- ln_diff("March") # 2 fcl2 values are not counted because they were not measured result_as_entered = NM
april_diff <- ln_diff("April") # 30 col values are missing
may_diff <- ln_diff("May")
june_diff <- ln_diff("June") #1 missing col value
july_diff <- ln_diff("July")
august_diff <- ln_diff("August")
dec_diff <- ln_diff("December")


col_year23 <- read_LIMS(
  start_date = "2023-02-01",
  end_date = "2023-03-01",
  parameter = "Coliforms Total (Colilert)",
  sample_class = c("Routine Daily", "Violation Check Samples","THMs/HAAs Monthly"),
  select_additional = "METHOD_USED"
) %>% group_by(METHOD_USED) %>% 
  summarise(count = n())
...
col_year <- col_year %>% mutate(
  METHOD_USED = ifelse(grepl("^SM", col_year$METHOD_USED),  "SM 9223 Colilert", col_year$METHOD_USED) 
) %>% filter(METHOD_USED != "SM 9223 Colilert")



unique(col_year$METHOD_USED)
#Method used can be entered in many different ways when analysts enter data. Need to find a different way to analyze data. 

#csv files do not include 7 specific dbp sites in chlorine and coliform results. Also do not include 7204 
